document.addEventListener('DOMContentLoaded', () => {
    ...
    // Admin sayfaya girdiðinde sohbet eden kullanýcýlarý getir
    async function loadChatUsers() {
        try {
            ...
            if (res.ok) {
                ...
                chatUsersList.innerHTML = '<div style="padding: 20px; text-align: center; color: #888;">Kullanýcýlar yüklenemedi.</div>';
            }
        } catch (err) {
            console.error(err);
        }
    ...
    async function openUserChat(user) {
        ...
        chatHeader.innerHTML = `<span>?? ${user.name} ile Görüþülüyor</span>`;
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (history.length === 0) {
                    chatMessages.innerHTML = '<div style="text-align: center; color: #999;">Sohbet geçmiþi yok. Ýlk mesajý gönderin!</div>';
                } else {
                    history.forEach(msg => {
            ...
            chatMessages.innerHTML = '<div style="text-align: center; color: #999;">Hata oluþtu.</div>';
        }
    }

    function addAdminMessageToUI(msg, type) {
        // Eðer placeholder varsa kaldýr
    ...
    async function sendAdminMessage() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Baþarýlýysa Socket ile müþteriye gönder
            ...
            console.error("Mesaj gönderim hatasý:", err);
        }
    }
    ...
    // Socket Dinleyicileri (admin panelinde window.socket tanýmlý mý kontrol edeceðiz veya burada tanýmlayacaðýz)
    // Eðer admin.html içinde <script src="/socket.io/socket.io.js"> varsa:
    ...
    if (window.socket) {
        window.socket.on('receive_message', (data) => {
            // Eðer müþteri admin'e yolladýysa
            if (data.receiver_id == adminUserId) {
                // Eðer ilgili müþterinin sohbeti açýksa
                if (currentChatUserId == data.sender_id) {
                    addAdminMessageToUI(data, 'received');
                } else {
                    // Kullanýcý listesinde kullanýcý yoksa yenile veya unread flag ekle (Basitçe listeyi yeniliyoruz)
                    loadChatUsers();
                    // Ses çalýnabiliyor
    ...
    // Ýlk yükleme
    // Sekme týklandýðýnda listeyi güncellemesini saðlamak için global switchTab'e kanca atýlabilir
    // Basitçe þimdilik 3 saniyede bir veya DOM yüklenince çalýþtýralým
