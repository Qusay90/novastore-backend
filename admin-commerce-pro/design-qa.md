...
- Güncel Vercel deployment: `dpl_FBRrohKcP239P63m7CYUfAi1eoMk`
...
- Authenticated fetch `HTTP 200` döndürdü. Deployed kaynak parmak izi `490ca0562e0903ae1bbe17bb591a3201dffb73298092b54773faf283712d47ec` yerel entegre/standalone artifact ile eşleşti; `noindex`, CSP, önizleme uyarısı ve güvenlik metinleri response içinde doğrulandı.
...
Kullanıcı, Work Mode cloud browser'ın yerel önizlemeyi ve Vercel Preview'ı açıp etkileşimli QA yapmasına açıkça izin verdi. Buna rağmen seçili Work Mode browser güvenlik katmanı `http://127.0.0.1:4173` açma eylemini kullanıcı isteğiyle çeliştiği gerekçesiyle reddetti ve localhost varyantı, raw CDP, başka browser yüzeyi veya dolaylı workaround kullanımını açıkça yasakladı. Yerel Vite sunucusu da ortamda `uv_interface_addresses returned Unknown system error 1` ile başlatılamadı. Önceki Vercel URL'si de aynı browser güvenlik katmanı tarafından reddedilmişti; güncel deployment authenticated HTTP fetch ile doğrulandı ancak yasaklı browser yolunu tekrar denemek veya farklı browser kullanmak yerine güvenlik sınırına uyuldu.
