...
function Status({ children }) {
  const key = String(children).toLocaleLowerCase("tr-TR").replaceAll(" ", "-");
...
function AppHeader({ title, contextOpen, onToggleContext, store, setStore, onCommand, onQuickCreate, onNotify, toast }) {
  return <>
    <header className="topbar">
      <div className="topbar-leading"><button className="icon-button rail-toggle" onClick={onToggleContext} aria-label={contextOpen ? "Bağlamsal menüyü daralt" : "Bağlamsal menüyü aç"} aria-expanded={contextOpen} aria-controls="context-navigation"><Icon name={contextOpen ? "back" : "menu"} /></button><div className="breadcrumb"><span>Çalışma Alanları</span><Icon name="right" /><strong>{title}</strong></div></div>
      ...
      <label className="store-select"><span className="sr-only">Mağaza kapsamı</span><Icon name="storefront" /><select value={store} onChange={(e) => setStore(e.target.value)} data-testid="store-scope"><option>Tüm Mağazalar · 24</option><option>NovaStore</option><option>TeknoPark · 2 mağaza</option><option>Eviva Home</option></select></label>
...
function IconRail({ active, setActive }) {
  return <nav className="icon-rail" aria-label="Ana çalışma alanları" data-testid="admin-sidebar">
    ...
    <div className="rail-nav">
      <button className={active === "dashboard" ? "active" : ""} aria-label="Pano" title="Pano" onClick={() => setActive("dashboard")} data-testid="nav-dashboard"><Icon name="house" /></button>
      <button className={active === "operations" ? "active" : ""} aria-label="Siparişler" title="Siparişler" onClick={() => setActive("operations")} data-testid="nav-orders"><Icon name="orders" /></button>
      <button className={active === "catalog" ? "active" : ""} aria-label="Ürünler" title="Ürünler" onClick={() => setActive("catalog")} data-testid="nav-products"><Icon name="package" /></button>
      <button className={active === "customers" ? "active" : ""} aria-label="Müşteriler" title="Müşteriler" onClick={() => setActive("customers")} data-testid="nav-customers"><Icon name="user" /></button>
      <button className={active === "sellers" ? "active" : ""} aria-label="Satıcılar" title="Satıcılar" onClick={() => setActive("sellers")} data-testid="nav-sellers"><Icon name="storefront" /></button>
      <button className={active === "finance" ? "active" : ""} aria-label="Finans" title="Finans" onClick={() => setActive("finance")} data-testid="nav-finance"><Icon name="card" /></button>
      <button className={active === "reports" ? "active" : ""} aria-label="Raporlar" title="Raporlar" onClick={() => setActive("reports")} data-testid="nav-reports"><Icon name="chart" /></button>
      <button className={active === "modules" ? "active" : ""} aria-label="Modüller" title="Modüller" onClick={() => setActive("modules")} data-testid="nav-modules"><Icon name="grid" /></button>
    </div>
    <div className="rail-bottom"><button className={active === "audit" ? "active" : ""} aria-label="Denetim" title="Denetim" onClick={() => setActive("audit")} data-testid="nav-audit"><Icon name="shield" /></button><button className={active === "settings" ? "active" : ""} aria-label="Ayarlar" title="Ayarlar" onClick={() => setActive("settings")} data-testid="nav-settings"><Icon name="settings" /></button><span className="avatar">AK</span></div>
  </nav>;
}

function ContextRail({ domain, open, savedViews, onView, onSaveView, onNavigate, onClose, notify }) {
  if (!open) return null;
  const title = domains.find((item) => item.id === domain)?.label;
  return <aside className="context-rail" id="context-navigation" data-testid="context-navigation">
    <div className="context-title"><h1>{title}</h1><Icon name="back" /></div>
    <label className="context-store"><Icon name="storefront" /><select aria-label="Örnek kapsam" onChange={(event) => notify(`Örnek kapsam “${event.target.value}” olarak seçildi.`)}><option>Tüm Mağazalar · 24</option><option>NovaStore</option></select></label>
    <nav className="context-nav" aria-label={`${title} bölümleri`}>{contextByDomain[domain].map(([label, count], index) => <button key={label} className={index === 1 && domain === "operations" ? "active" : ""} onClick={() => notify(`“${label}” alt görünümü önizleme kapsamında seçildi.`)}><Icon name={index === 0 ? "house" : index === 1 ? "orders" : "right"} /><span>{label}</span>{count && <b>{count}</b>}</button>)}</nav>
    {(domain === "operations" || domain === "catalog") && <section className="saved-views"><header><strong>Kaydedilmiş Görünümler</strong><button className="icon-button small" onClick={onSaveView} aria-label="Görünüm kaydet"><Icon name="plus" /></button></header>{savedViews.map((view) => <button key={view.name} onClick={() => onView(view)}><Icon name="bookmark" /><span>{view.name}</span></button>)}</section>}
...
function Operations({ orders, setOrders, selected, setSelected, currentId, setCurrentId, filter, setFilter, tab, setTab, editingLayout, setEditingLayout, savedViews, onSaveView, notify }) {
  ...
  const bulkStatus = (status) => { setOrders((rows) => rows.map((row) => selected.includes(row.id) ? { ...row, status } : row)); notify(`${selected.length} örnek sipariş önizlemede güncellendi.`); setSelected([]); };
  ...
  return <div className={`workspace operations-workspace ${editingLayout ? "is-editing" : ""}`}>
    <div className="workspace-heading"><div><span className="eyebrow">Etkileşimli önizleme</span><h2>Sipariş Operasyon Masası</h2><p>Örnek mağaza, sipariş, satıcı ve SLA akışıyla ana operasyon deneyimini inceleyin.</p></div><div className="heading-actions"><select aria-label="Görünüm">{savedViews.map((view) => <option key={view.name}>{view.name}</option>)}</select><button className={editingLayout ? "secondary-button active" : "secondary-button"} onClick={() => setEditingLayout(!editingLayout)} data-testid="layout-edit"><Icon name="grid" />{editingLayout ? "Düzenlemeyi bitir" : "Düzeni düzenle"}</button><button className="secondary-button" onClick={onSaveView}><Icon name="bookmark" />Görünümü kaydet</button></div></div>
    ...
    <section className="insight-grid">
      <article className="module-card revenue-module"><header><div><span className="eyebrow">Son 7 gün · örnek</span><h3>Net Ciro</h3></div><button className="icon-button small" aria-label="Net ciro modülü seçeneklerini önizle" onClick={() => notify("Net ciro modülü seçenekleri bu önizlemede bilgilendirme amaçlıdır.")}><Icon name="menu" /></button></header><div className="trend-summary"><strong>₺2,04 Mn</strong><span className="positive">↑ %14,6</span></div><div className="trend-table" aria-label="Örnek günlük net ciro"><span>7 Tem<b>₺410B</b></span><span>8 Tem<b>₺780B</b></span><span>9 Tem<b>₺640B</b></span><span>10 Tem<b>₺1,1M</b></span><span>11 Tem<b>₺1,0M</b></span><span>12 Tem<b>₺1,5M</b></span><span>13 Tem<b>₺2,0M</b></span></div></article>
      <article className="module-card distribution-module"><header><h3>Sipariş Durum Dağılımı</h3><button className="icon-button small" aria-label="Sipariş dağılımı modülü seçeneklerini önizle" onClick={() => notify("Sipariş dağılımı modülü seçenekleri bu önizlemede bilgilendirme amaçlıdır.")}><Icon name="menu" /></button></header>{[["Yeni", 2214, 32], ["Hazırlanıyor", 1876, 27], ["Kargoya Verildi", 1642, 24], ["Teslim Edildi", 876, 13], ["İptal / İade", 234, 4]].map(([label, value, percent]) => <div className="distribution-row" key={label}><span>{label}</span><progress max="100" value={percent} /><b>{value.toLocaleString("tr-TR")}</b><small>%{percent}</small></div>)}</article>
    </section>
    <section className="ledger-card">
      <nav className="ledger-tabs" aria-label="Operasyon kayıt türleri">{[["orders", "Siparişler", 28], ["seller-orders", "Satıcı Siparişleri", 42], ["returns", "İadeler", 7], ["stock", "Stok", 14]].map(([id, label, count]) => <button className={tab === id ? "active" : ""} onClick={() => setTab(id)} key={id} data-testid={`ledger-tab-${id}`}>{label}<b>{count}</b></button>)}</nav>
      {tab !== "orders" ? <div className="state-panel"><Icon name={tab === "returns" ? "back" : "package"} /><h3>{tab === "returns" ? "İade talepleri" : tab === "stock" ? "Stok riskleri" : "Satıcı siparişleri"}</h3><p>Bu görünüm seçili mağaza kapsamına göre hazırlandı. Örnek kayıtlar operasyon tasarımının bir parçasıdır.</p><button className="primary-button" onClick={() => setTab("orders")}>Siparişlere dön</button></div> : <>
        <div className="ledger-toolbar">
          {selected.length > 0 ? <div className="bulk-toolbar" data-testid="bulk-actions"><strong>{selected.length} sipariş seçildi</strong><button onClick={() => setSelected([])}>Seçimi temizle</button><select aria-label="Toplu durum güncelle" defaultValue="" onChange={(e) => e.target.value && bulkStatus(e.target.value)}><option value="">Durumu güncelle</option><option>Hazırlanıyor</option><option>Kargoya Verildi</option><option>İptal Edildi</option></select><button onClick={() => notify(`${selected.length} örnek sipariş için satıcı atama akışı önizlendi.`)}>Satıcı atamayı önizle</button></div> : <div className="filter-toolbar"><label className="table-search"><Icon name="search" /><input type="search" value={filter.query} onChange={(e) => setFilter({ ...filter, query: e.target.value })} placeholder="Sipariş, müşteri veya ürün ara" aria-label="Siparişlerde ara" data-testid="filter-search" /></label><select value={filter.status} onChange={(e) => setFilter({ ...filter, status: e.target.value })} data-testid="status-filter"><option>Tümü</option><option>Yeni</option><option>Hazırlanıyor</option><option>Kargoya Verildi</option><option>Teslim Edildi</option></select><button className="secondary-button" onClick={() => setFilter({ status: "Tümü", query: "" })}><Icon name="filter" />Filtreleri temizle</button></div>}
        </div>
        <div className="table-scroll"><table className="data-table"><thead><tr><th><input type="checkbox" aria-label="Tüm siparişleri seç" checked={allSelected} onChange={(e) => setSelected(e.target.checked ? visible.map((order) => order.id) : [])} /></th><th>Sipariş ID</th><th>Satıcı / Mağaza</th><th>Müşteri</th><th>Ürün</th><th>Kanal</th><th>Tutar</th><th>Durum</th><th>SLA Yaşı</th><th>Sahip</th><th><span className="sr-only">İşlem</span></th></tr></thead><tbody>{visible.map((order) => <tr key={order.id} className={current?.id === order.id ? "selected-row" : ""} tabIndex="0" onClick={(e) => openInspector(order.id, e.target.closest("button") || e.currentTarget)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openInspector(order.id, e.currentTarget); } }} data-testid="table-row" data-row-id={order.id}><td onClick={(e) => e.stopPropagation()}><input type="checkbox" aria-label={`${order.id} satırını seç`} data-testid="row-select" checked={selected.includes(order.id)} onChange={(e) => setSelected(e.target.checked ? [...selected, order.id] : selected.filter((id) => id !== order.id))} /></td><td><strong>{order.id}</strong></td><td><span className="seller-cell"><Icon name="storefront" />{order.seller}</span></td><td>{order.customer}</td><td><span className="product-cell"><img src={order.image} alt="" />{order.product}</span></td><td>{order.channel}</td><td><strong>{money(order.amount)}</strong></td><td><Status>{order.status}</Status></td><td className="sla">{order.age}</td><td>{order.owner}</td><td><button className="icon-button small" aria-label={`${order.id} siparişini incele`}><Icon name="menu" /></button></td></tr>)}</tbody></table></div>
...
export function App() {
  ...
  const notify = (message) => { setToast(message); window.clearTimeout(notify.timer); notify.timer = window.setTimeout(() => setToast(""), 2800); };
  ...
  const navigate = (next) => { setDomain(next); setDialog(null); setCurrentId(next === "operations" && window.innerWidth > 760 ? "NS-10482" : ""); if (window.innerWidth <= 760) setContextOpen(false); };
  useEffect(() => {
    const onKey = (event) => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLocaleLowerCase() === "k") { event.preventDefault(); openDialog("command"); }
      if (event.key === "Escape" && contextOpen && !dialog) setContextOpen(false);
    };
  ...
  const body = {
    dashboard: <Operations orders={orders} setOrders={setOrders} selected={selected} setSelected={setSelected} currentId={currentId} setCurrentId={setCurrentId} filter={filter} setFilter={setFilter} tab={tab} setTab={setTab} editingLayout={editingLayout} setEditingLayout={setEditingLayout} savedViews={savedViews} onSaveView={() => openDialog("save-view")} notify={notify} />,
  ...
  return <div className={`admin-shell ${contextOpen ? "context-open" : "context-closed"}`} data-testid="admin-shell">
    ...
    <ContextRail domain={domain} open={contextOpen} savedViews={savedViews} onView={(view) => { setFilter({ status: view.status, query: "" }); navigate("operations"); notify(`“${view.name}” örnek görünümü uygulandı.`); }} onSaveView={() => openDialog("save-view")} onNavigate={navigate} onClose={() => setContextOpen(false)} notify={notify} />
    {contextOpen && <button className="context-scrim" aria-label="Bağlamsal menüyü kapat" onClick={() => setContextOpen(false)} data-testid="context-scrim" />}
    <div className="admin-main"><AppHeader title={title} contextOpen={contextOpen} onToggleContext={() => setContextOpen(!contextOpen)} store={store} setStore={(next) => { setStore(next); notify(`“${next}” örnek mağaza kapsamı seçildi.`); }} onCommand={() => openDialog("command")} onQuickCreate={() => openDialog("quick-create")} onNotify={notify} toast={toast} /><main className="content-area"><PreviewBanner />{body}</main><footer className="statusbar"><span>Commerce Pro tasarım önizlemesi</span><span>Örnek veri · kalıcı kayıt yok</span><span>API ve ödeme bağlantıları kapalı</span><button onClick={() => notify("Örnek arayüz durumu yenilendi.")}><Icon name="refresh" />Önizlemeyi yenile</button></footer></div>
